...
case class Audiobooks(
                     ...
                     total: Int,
                     items: List[SimpleAudiobookObj]
                     )
object Audiobooks{
  implicit val jsonFormat: RootJsonFormat[Audiobooks] =
    new RootJsonFormat[Audiobooks] {
      override def write(obj: Audiobooks): JsValue = ???

      override def read(json: JsValue): Audiobooks = {
        ...
        Audiobooks(
          ...
          items = fields.apply("items").convertTo[List[SimpleAudiobookObj]]
        )
      }
    }
}
